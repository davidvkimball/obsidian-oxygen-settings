...
## When AI Agents Should Check .ref Setup

**Only check `.ref` folder when user explicitly requests:**
- API lookups ("what does the Obsidian API say?", "check the latest API")
- Documentation references ("what does the Obsidian documentation say?", "look up in docs")
- Latest features ("what's new in the API?", "check latest Obsidian docs")

**Trigger phrases:**
- "latest API" / "latest Obsidian API"
- "what does the Obsidian documentation say"
- "check the Obsidian docs"
- "look up in obsidian-api"
- "what's in the sample plugin"
- "check obsidian-api for..."
- Similar explicit documentation/API requests

**Quick check:**
- Windows: `Test-Path .ref/obsidian-api`
- Unix: `test -d .ref/obsidian-api`

**If missing, run setup script immediately before proceeding with the lookup:**
- Windows: `.\scripts\setup-ref-links.ps1`
- Unix: `./scripts/setup-ref-links.sh`
